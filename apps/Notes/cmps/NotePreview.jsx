...
export function NotePreview({ note,onRemove, onSelectNote,onPinNote}) {
    ...
    return <article  className={(note.isPinned)? "note-pinned note-area":"note-area"}>
        ...
        <button className={(note.isPinned)?'pin-btn currently-pinned':'pin-btn'} title="Pin/Unpin Note" onClick={(ev) => {onPinNote(ev,note.id)}}>*</button>   
        </div>
    </article>
}